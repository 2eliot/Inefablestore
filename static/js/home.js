...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Infinite rotate helpers
  function rotateForward(r) {
  ...
  function railStep(r) {
    ...
    if (!items || items.length === 0) return Math.max(0, r ? r.clientWidth - 40 : 0);
    if (items.length === 1) return items[0].offsetWidth;
    const a = items[0];
    const b = items[1];
    // Distance from one card to the next (includes gap)
    const delta = Math.abs(b.offsetLeft - a.offsetLeft) || (a.offsetWidth);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (dir > 0) rotateForward(r); else rotateBackward(r);
  });
});
